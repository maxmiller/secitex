...
=begin
    if usuario
      if usuario.participante?
        ...
        if usuario.organizador? && !usuario.servidor? # Organizador externo
          return usuario if usuario.authenticate(senha)
        else
          begin # Organizador servidor
            return usuario if SUAP::API.authenticate(username: usuario.matricula, password: senha)
